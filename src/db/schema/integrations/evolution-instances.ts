...
/**
 * Instâncias Evolution API; tenant_id para RLS e queries.
 */
export const evolutionInstances = pgTable(
  ...
  (t) => ({
    evolution_instances_tenant_external_unique: unique(
      "evolution_instances_tenant_external_unique"
    ).on(t.tenantId, t.externalId),
    evolution_instances_tenant_external_idx: index(
      "evolution_instances_tenant_external_idx"
    ).on(t.tenantId, t.externalId),
  })
);
